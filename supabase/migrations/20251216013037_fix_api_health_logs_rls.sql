-- ==========================================
-- FIX: Políticas RLS para api_health_logs
-- ==========================================
-- Asegurar que las políticas RLS permitan que el service_role 
-- pueda insertar y leer logs, y que los admins puedan leerlos
-- Esto es crítico para que el sistema de monitoreo de APIs funcione
...
-- Eliminar TODAS las políticas existentes (cualquier variación de nombre)
DO $$ 
DECLARE
    r RECORD;
BEGIN
    FOR r IN (SELECT policyname FROM pg_policies WHERE schemaname = 'public' AND tablename = 'api_health_logs') 
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON api_health_logs', r.policyname);
    END LOOP;
END $$;

-- Política 1: Service role puede insertar logs
-- CRÍTICO: Sin esto, el backend no puede registrar intentos de API
-- Usar WITH CHECK (true) para permitir cualquier inserción desde service_role
...
-- Usar USING (true) para permitir leer cualquier fila desde service_role
...
COMMENT ON POLICY "service_role_insert_api_health_logs" ON api_health_logs IS 
...
COMMENT ON POLICY "service_role_select_api_health_logs" ON api_health_logs IS 
'Permite que el service_role (backend) lea todos los logs. CRÍTICO para que el endpoint /api/exchange-rate/health funcione.';

COMMENT ON POLICY "admins_select_api_health_logs" ON api_health_logs IS 
