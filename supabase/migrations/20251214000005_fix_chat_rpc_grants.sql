-- FIX: Grants and Search Path for get_chat_conversations_v2
-- Issue: 400 Bad Request often means the function is not found (due to permissions) or signature mismatch.

-- 1. Ensure clean slate
DROP FUNCTION IF EXISTS public.get_chat_conversations_v2(uuid);

-- 2. Recreate with robust settings
CREATE OR REPLACE FUNCTION public.get_chat_conversations_v2(p_user_id uuid)
 ...
 LANGUAGE plpgsql
 SECURITY DEFINER
 -- Fix: Unquoted search path to be safe, include auth
...
-- 3. Explicit Grants (CRITICAL for PostgREST access)
...
GRANT EXECUTE ON FUNCTION public.get_chat_conversations_v2(uuid) TO anon; -- Opcional, pero authenticated es el importante

COMMENT ON FUNCTION public.get_chat_conversations_v2(uuid) IS 'Versión corregida v5: Grants explícitos y search_path seguro.';
